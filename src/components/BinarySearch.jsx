...
const BinarySearch = () => {
    ...
    const setup = (p5, canvasParentRef) => {
        ...
        p5.frameRate(1); // slower for visualization
    };
    ...
    const draw = (p5) => {
        ...
        for (let i = 0; i < values.length; i++) {
            if (i === mid && found) {
                p5.fill(0, 255, 0); // Green if found
            } else if (i === mid) {
                p5.fill(255, 204, 0); // Yellow for current mid
            } else {
                p5.fill(240);
            }
        ...
        if (started && low <= high && !found) {
            mid = Math.floor((low + high) / 2);
            if (values[mid] === parseInt(target)) {
                found = true;
                document.getElementById("output-binary").
                    innerHTML = `<span style="color:green;">:) Found at index ${mid}</span>`;
    ...
    return (
        <div style={{ marginBottom: "40px" }}>
            ...
            <div id="output-binary" style={{ marginTop: "10px", fontSize: "16px" }}></div>
        </div>
    );
};
